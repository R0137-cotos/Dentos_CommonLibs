...
public interface CommonMasterRepository extends CrudRepository<CommonMaster, Long> {
	...
	// 受注完了時のメール送信でのみ使用可能
	@Query(value = "SELECT * FROM common_master WHERE (service_category = :SERVICE_CATEGORY OR (service_category = '0' AND column_name NOT IN (SELECT column_name FROM common_master WHERE service_category = :SERVICE_CATEGORY))) AND COLUMN_NAME = :COLUMN_NAME AND delete_flg = '0' order by id", nativeQuery = true)
