...
public interface ContractChecklistCompMasterRepository extends CrudRepository<ContractChecklistCompMaster, Long> {
	...
	@Query(value = "FROM ContractChecklistCompMaster WHERE product_master_id = :PRODUCT_MASTER_ID AND target_contract_type = :TARGET_CONTRACT_TYPE AND target_lifecycle_status = :TARGET_LIFECYCLE_STATUS order by id")
