...
public interface CommonMasterDetailRepository extends CrudRepository<CommonMasterDetail, Long> {
	...
	@Query(value = "SELECT * FROM common_master_detail WHERE common_master_id = (select id from common_master where column_name = :COLUMN_NAME and rownum <= 1) AND to_date(:AVAILABLE_PERIOD, 'yyyymmdd') between trunc(available_period_from) and trunc(available_period_to) AND delete_flg = 0", nativeQuery = true)
	public List<CommonMasterDetail> findByCommonMasterColumnNameAndAvailablePeriodBetween(@Param("COLUMN_NAME") String columnName, @Param("AVAILABLE_PERIOD") String availablePeriod);
