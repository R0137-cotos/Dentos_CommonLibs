...
public interface CommonMasterDetailRepository extends CrudRepository<CommonMasterDetail, Long> {
	@Query(value = "FROM CommonMasterDetail WHERE common_master_id = :COMMON_MASTER_ID AND trunc(current_date) between trunc(available_period_from) and trunc(available_period_to) AND delete_flg = 0 order by sort_number")
