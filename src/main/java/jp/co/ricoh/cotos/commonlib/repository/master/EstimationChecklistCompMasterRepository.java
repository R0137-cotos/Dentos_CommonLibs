...
public interface EstimationChecklistCompMasterRepository extends CrudRepository<EstimationChecklistCompMaster, Long> {
	...
	@Query(value = "FROM EstimationChecklistCompMaster WHERE product_master_id = :PRODUCT_MASTER_ID AND (target_estimation_type = '1' OR target_estimation_type = :TARGET_ESTIMATION_TYPE) AND target_lifecycle_status = :TARGET_LIFECYCLE_STATUS order by id", nativeQuery = true)
