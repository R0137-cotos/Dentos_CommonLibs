...
@Component
public class EmployeeConListener {
	...
	public void setEmployeeMasterRepository(EmployeeMasterRepository employeeMasterRepository) {
		EmployeeConListener.employeeMasterRepository = employeeMasterRepository;
	}
	...
	@PrePersist
	@Transactional
	public void appendsEmployeeFields(EmployeeCon employeeCon) {
		EmployeeMaster employeeMaster = employeeMasterRepository.findByMomEmployeeId(employeeCon.getEmployeeMaster().getMomEmployeeId());

		if (employeeMaster == null) {
			throw new IllegalArgumentException("Target Not Found By ID : EmployeeMaster");
		}

		BeanUtils.copyProperties(employeeMaster, employeeCon);
		employeeCon.setEmployeeName(employeeMaster.getJobname1() + employeeMaster.getJobname2());
		employeeCon.setAddress(convertJoinedAddress(employeeMaster));
		employeeCon.setEmployeeMaster(employeeMaster);
	}
