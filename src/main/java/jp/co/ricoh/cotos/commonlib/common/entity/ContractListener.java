...
public class ContractListener {
	...
	public void appendsContractNumber(Contract contract) {
		...
		long sequence = dbUtil.loadSingleFromSQLFile("sql/nextContractNumberSequence.sql", GeneratedNumberContract.class).getGeneratedNumber();
		...
		while (todayLong > sequence) {
			...
			dbUtil.loadFromSQLFile("sql/updateContractNumberVal.2.sql", GeneratedNumberContract.class);
			dbUtil.execute("sql/updateContractNumberVal.3.sql", Collections.emptyMap());
			sequence = dbUtil.loadSingleFromSQLFile("sql/nextContractNumberSequence.sql", GeneratedNumberContract.class).getGeneratedNumber();
		}
