...
public class TestSendMailFail {
	...
	private void メール送信履歴確認(boolean isApiExce) {
		...
		Assert.assertEquals("エラーフラグに「1」が設定されていること", true, sendMailHistory.isErrorFlg());
		Assert.assertEquals("エラー内容が設定されていないこと", ErrorContent.通信エラー, sendMailHistory.getErrorContent());
